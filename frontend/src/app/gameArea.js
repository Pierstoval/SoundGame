...
class GameArea {
  ...
  update() {
    this.picks && this.picks.destroy();
    this.picks = this.game.add.group();
    ...
    if (currentUser) {
      var headX = this.game.world.centerX + currentUser.pick.bodyParts[0].center.x;
      var headY = this.game.world.centerY + currentUser.pick.bodyParts[0].center.y;
  ...
  drawPick(player) {
    ...
    for (const bodyPart of player.pick.bodyParts) {
      ...
      if (first) {
        ...
        item.angle = 270 - player.pick.headAngle;
      } else {
        item = this.game.add.sprite(
          this.game.world.centerX + bodyPart.center.x,
          this.game.world.centerY + bodyPart.center.y,
    ...
    for (item of items.reverse()) {
      this.picks.bringToTop(item);
    }
  }

  destroyPick(pickGraphics) {
    this.game.add.tween(pickGraphics.scale)
      .to( {x: 1.2, y: 1.2}, 1000, Phaser.Easing.Back.InOut, true, 0, false)
      .yoyo(true);

    this.game.add.tween(pickGraphics)
    ...
    let destroyCallback = destroy.bind(this, pickGraphics);
